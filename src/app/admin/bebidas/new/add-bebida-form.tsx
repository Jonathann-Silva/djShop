...
export function AddBebidaForm() {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<BebidaFormValues>({
    defaultValues: {
      name: "",
      brand: "",
      profitMargin: 0,
      description: "",
      imageId: `bebida-${Math.floor(Math.random() * 100)}`,
      onSale: false,
      priceUrl: "",
      imageUrl: "",
    },
  ...
  const onSubmit = async (data: BebidaFormValues) => {
    setIsSaving(true);
    const dataToSubmit = {
      ...data,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <CardFooter className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={isSaving}>{isSaving ? 'Salvando...' : 'Adicionar Bebida'}</Button>
      </CardFooter>
    </form>
  );
}
