...
export function EditElectronicForm({ product }: EditElectronicFormProps) {
  ...
  const onSubmit = async (data: Eletronico) => {
    setIsSaving(true);

    const dataToSubmit: Eletronico = {
      ...data,
      profitMargin: Number(data.profitMargin) || 0,
    };

    try {
      const result = await updateElectronic(dataToSubmit);
      setIsSaving(false);

      if (result.success) {
        toast({
          title: "Eletrônico Atualizado!",
