...
export default function AdminEletronicosPage() {
  const { user, loading: authLoading } = useAuth();
  const router = useRouter();
  const { toast } = useToast();
  const [products, setProducts] = useState<Eletronico[]>([]);
  const [loading, setLoading] = useState(true);
  const [sortOrder, setSortOrder] = useState("name-asc");
  const [isManageBrandsOpen, setIsManageBrandsOpen] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const [productToDelete, setProductToDelete] = useState<Eletronico | null>(null);


  useEffect(() => {
    async function fetchProducts() {
      const fetchedProducts = await getElectronics();
  ...
  const handleDeleteProduct = async () => {
    ...
    setIsDeleting(true);
    const result = await removeElectronic(productToDelete.id);
    setIsDeleting(false);

    if (result.success) {
        toast({
            title: "Eletrônico Removido",
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-headline font-bold">
            Gerenciar Eletrônicos
          </h1>
          <p className="mt-2 text-lg text-muted-foreground">
            Adicione, edite ou remova eletrônicos do seu catálogo.
          </p>
        </div>
         <div className="flex items-center gap-4">
            ...
            <Button asChild>
              <Link href="/admin/eletronicos/new">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                           <Link href={`/admin/eletronicos/${product.id}/edit`}>Editar</Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem 
